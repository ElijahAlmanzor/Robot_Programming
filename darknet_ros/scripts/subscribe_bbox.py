...
#Yolo was transfer learnt from YoloV3 weights provided by https://github.com/pjreddie/darknet
#It was trained using 700 images taken from Gazebo simulation which were all manually labelled using the bounding box tool provided in the link above
#It was trained on GoogleCollab for around 8000 batches (as they recommended)
...
class display_bbox:
    ...
    def image_callback(self, data):
        ...
        #A list which stores whether the current array of bounding boxes received actually contains any weeds
        self.weed_list = list()

        #In a try as it crashes when self.current_bboxes is still empty (e.g. when it hans't received bounding boxes yet)
        try:
            #For loop to display bounding boxes 
            for bbox in self.current_bboxes:

                #The bounding boxes are outputted via their corners relative to the whole image
                ...
                #Just plots the current bounding box in the image figure/window
                ...
                #As it then allows for robotic arm spraying of the weeds in possible future work
                #At the moment, if it is detecting weed close to the centre of the camera image then send signal to other node to spray
                
                #If the object detected is a weed, add to the weed_list
                if bbox.Class == 'weed':
                    ...
                    #Just some trial and error values that worked close enough to the centre of the image
                ...
                ''' Some extra in case theres a robot arm, it needs to be put in a pose array to be displayed in Rviz
            ...
            print("No Bounding Box detected just yet")
        ...
        #Publish the annotated images with the bounding boxes for viewing in Rviz
